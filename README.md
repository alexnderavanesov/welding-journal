...
Команда применения миграций работает только из ветки `main` и останавливается до подключения к базе из любой другой ветки или при detached HEAD. После миграции команда также безопасно заполняет `ДНО` в полях дефектов ВИК, УЗК и ПВК для уже сохраненных годных результатов. Заполнение выполняется один раз; последующие миграции проверяют служебную отметку без повторного полного прохода по таблицам.
...
Для удаленной базы эта же команда после миграций выполняет соответствующее заполнение старых годных результатов.

`db:remote-migration` не использует `DATABASE_URL` и завершится с ошибкой, если `DATABASE_URL_REMOTE_FOR_MIGRATIONS` не задана. Перед любым подключением команда дополнительно проверяет текущую Git-ветку и разрешает применение только из `main`; из рабочей, экспериментальной ветки или detached HEAD база не изменяется. Рабочий Docker-образ не содержит исходный код, зависимости проекта или Drizzle CLI, поэтому миграцию запускают из локальной копии репозитория, а не из терминала контейнера. Не добавляйте production-миграции в start command и не коммитьте настоящую строку подключения удаленной базы.
