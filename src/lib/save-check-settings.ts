...
const OFFICIAL_STAMP_SAVE_CHECK_ITEMS: SaveCheckSettingItem[] = [
  ...
  },
  {
    ...
    description: 'Для обычного стыка проверяется наибольший из D1/D2. У соединения «У…» проверяется диаметр меньшего элемента; при равных диаметрах проверяется общее значение D.',
    example: 'Для C18 с D1 = 99 и D2 = 105 проверяется D105. Для У17 с D1 = 530 и D2 = 25 проверяется D25.',
  },
  {
    id: 'officialThickness',
    label: 'Толщины T1/T2',
    description: 'Для обычного стыка проверяется наибольшая из T1/T2. У соединения «У…» проверяется T того же меньшего элемента; при равных диаметрах проверяются обе толщины.',
    example: 'Для C17 с T1 = 5 и T2 = 6 проверяется T6. Для У17 с D1/T1 = 530/30 и D2/T2 = 25/3 проверяется связанная пара D25/T3.',
  },
  {
    id: 'officialDls',
