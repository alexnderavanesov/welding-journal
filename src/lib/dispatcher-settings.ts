  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        description: 'Показывать задачи несогласованных назначений ВИК, РК, УЗК, ПВК и ТВМТ на 100% линии. Для У-стыков РК, УЗК и ПВК считаются взаимозаменяемыми. РФА, СТЛС и МКК не сравниваются; ПСТО проверяется отдельно.',
      },
      {
...
export const DISPATCHER_SETTING_HELP: Record<DispatcherSettingId, { meaning: string; example: string }> = {
  ...
  'line-control-presence': {
    meaning: 'На 100% линиях проверяет единое назначение ВИК, РК, УЗК, ПВК и ТВМТ. На обычных стыках набор «да» должен совпадать. На стыке типа «У…» назначения РК, УЗК и ПВК взаимозаменяемы: достаточно хотя бы одного. ВИК и ТВМТ сравниваются точно. РФА, СТЛС и МКК могут назначаться отдельным стыкам и в проверку линии не входят; ПСТО проверяется отдельно.',
