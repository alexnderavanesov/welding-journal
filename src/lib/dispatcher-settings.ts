  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        description: 'Показывать задачи несогласованных назначений ЛНК на 100% линии. Для У-стыков РК, УЗК и ПВК считаются взаимозаменяемыми. ПСТО проверяется отдельно.',
      },
      {
...
export const DISPATCHER_SETTING_HELP: Record<DispatcherSettingId, { meaning: string; example: string }> = {
  ...
  'line-control-presence': {
    meaning: 'На 100% линиях проверяет единое назначение ЛНК. На обычных стыках набор «да» должен совпадать. На стыке типа «У…» назначения РК, УЗК и ПВК взаимозаменяемы: достаточно хотя бы одного. ВИК и другие методы по-прежнему сравниваются точно; ПСТО проверяется отдельно.',
